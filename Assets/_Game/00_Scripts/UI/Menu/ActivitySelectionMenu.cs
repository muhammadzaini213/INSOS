...
namespace Slafurry.UI.Menu
{
    ...
    public class ActivitySelectionMenu : MonoBehaviour
    {
        ...
        [SerializeField] private Button morningActivityButton;
        
        [Tooltip("Cleaning button (Section 3)")]
        [SerializeField] private Button cleaningButton;
        
        [Tooltip("Card Game button (Section 2)")]
        [SerializeField] private Button cardGameButton;
        
        [Header("Continue Indicators (Optional)")]
        [Tooltip("Show this icon/text when Section 1 has checkpoint (e.g., '▶ Continue')")]
        [SerializeField] private GameObject section1ContinueIcon;
        
        [Tooltip("Show this icon/text when Section 2 has checkpoint")]
        [SerializeField] private GameObject section2ContinueIcon;
        
        [Tooltip("Show this icon/text when Section 3 has checkpoint")]
        [SerializeField] private GameObject section3ContinueIcon;
        ...
        private void UpdateContinueIndicators()
        {
            ...
            {
                if (showDebugLogs)
                    Debug.Log("[ActivitySelectionMenu] CheckpointManager not ready yet, skipping indicator update");
                return;
            }
            ...
            if (section1ContinueIcon != null)
            {
                ...
                if (showDebugLogs)
                    Debug.Log($"[ActivitySelectionMenu] Section 1 continue indicator: {hasCheckpoint}");
            }
            ...
            if (section2ContinueIcon != null)
            {
                ...
                if (showDebugLogs)
                    Debug.Log($"[ActivitySelectionMenu] Section 2 continue indicator: {hasCheckpoint}");
            }
            ...
            if (section3ContinueIcon != null)
            {
                ...
                if (showDebugLogs)
                    Debug.Log($"[ActivitySelectionMenu] Section 3 continue indicator: {hasCheckpoint}");
            }
        }
        ...
        public void OnMorningActivityClicked()
        {
            ...
            if (showDebugLogs)
                Debug.Log($"[ActivitySelectionMenu] Morning Activity clicked → Loading: {sceneToLoad}");
            
            SceneSystem.Load(sceneToLoad);
        }
        
        /// <summary>
        /// Card Game button callback (Section 2).
        ...
        /// <summary>
        /// Cleaning button callback (Section 3).
